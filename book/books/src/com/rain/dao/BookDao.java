...
public class BookDao {
	 ...
	 * @param outnum
	 */
	public void addBook(String card, String name, String type, String autho, String press, int num,int innum,int outnum) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "insert  into book(card,name,type,autho,press,num,innum,outnum) values(?,?,?,?,?,?)";
		int rs = 0;
	...
	public ArrayList<BookBean> get_ListInfo(){
		...
		try {
			...
			while(rs.next()){
				...
				tag.setInNum(rs.getInt("innum"));
				tag.setOutNum(rs.getInt("outnum"));
				tag_Array.add(tag);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	 ...
	 * 修改图书的信息，bid作为条件，
	 */
	public void updateBook(int bid, String card, String name, String type, String autho, String press, int num,int innum,int outnum) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "update book set name=?,card=?,type=?,autho=?,press=?,num=?,innum=?,outnum=? where bid=?";
		PreparedStatement stm = null;
		try {
			stm = conn.prepareStatement(sql);
	...
	public ArrayList<BookBean> getLikeList(String name) {
		...
		ArrayList<BookBean> tag_Array = new ArrayList<BookBean>();
		Connection conn = DBUtil.getConnectDb();
		String sql = "select * from book where name like '%"+name+"%'"; //模糊查询
		PreparedStatement stm = null;
		ResultSet rs = null;
		try {
			stm = conn.prepareStatement(sql);
	...
	public void borrowBook(int bid, AdminBean adminbean) {
		...
		Connection conn = DBUtil.getConnectDb();
		String sql1 = "insert  into history(aid,bid,card,bookname,adminname,username,begintime,endtime,status) values(?,?,?,?,?,?,?,?,?)";
		int rs1 = 0;
		PreparedStatement stm1 = null;
		try {
			stm1 = conn.prepareStatement(sql1);
			stm1.setInt(1, adminbean.getAid());
			stm1.setInt(2, bookbean.getBid());
			stm1.setString(3, bookbean.getCard());
			stm1.setString(4, bookbean.getName());
			stm1.setString(5, adminbean.getUsername());
			stm1.setString(6, adminbean.getName());
			stm1.setString(7, begintime);
			stm1.setString(8, endtime);
			stm1.setInt(9, 1);
		...
		//在馆数量减一，外借数量加一
		String sql2="update book set innum=?,outnum=? where bid=?";
		PreparedStatement stm2 = null;
		try {
			stm2= conn.prepareStatement(sql2);
			stm2.setInt(1, bookbean.getInNum()-1);
			stm2.setInt(2, bookbean.getOutNum()+1);
	...
	public void borrowBook2(int hid) {
		...
		HistoryBean tag = new HistoryBean();
		String sql1= "select bid from history where hid="+hid;
		PreparedStatement stm1 = null;
		ResultSet rs = null;
		try {
			stm1= conn.prepareStatement(sql1);
		...
		//在馆数量减一，外借数量加一
		BookBean bookbean = new BookBean();
		bookbean = this.get_BookInfo(hbid);
		String sql2="update book set innum=?,outnum=? where bid=?";
		PreparedStatement stm2 = null;
		try {
			stm2= conn.prepareStatement(sql2);
